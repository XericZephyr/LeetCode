...
#                     break
#             if T > 0:
#                 return max(op(last_i+1, current_profit + T - last_p), op(last_i+2, current_profit+T))
#             else:
#                 return op(start+1, current_profit)
#
#         return op(0, 0)
...
class Solution(object):
    def maxProfit(self, prices):
        ...
        for i in range(len(prices)):
            ...
            cool_down = hold+prices[i]
            hold = max(hold, ready_to_buy-prices[i])
            ready_to_buy = max(prev_cool_down, ready_to_buy)

        return max(cool_down, ready_to_buy)
...
if __name__ == '__main__':
    ...
    print Solution().maxProfit([70,4,83,56,94,72,78,43,2,86,65,100,94,56,41,66,3,33,10,3])
